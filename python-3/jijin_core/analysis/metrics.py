"""
绩效指标计算
从各脚本提炼的统一绩效指标函数：年化收益、最大回撤、波动率、
夏普比率、索提诺、卡尔马、跟踪误差/偏离、信息比率、上下行捕获等。
"""
...
def calc_tracking_error_and_difference(fund_returns: pd.Series, index_returns: pd.Series) -> dict:
    """计算跟踪误差（TE）与跟踪偏离度（TD）。"""
...
def calc_information_ratio(fund_annual_ret: float, index_annual_ret: float, tracking_error: float) -> float:
    """计算信息比率。"""
...
def calc_monthly_win_rate(nav_series: pd.Series, benchmark: pd.Series | None = None) -> dict:
    ...
    series = _as_datetime_index_vals(nav_series)
...
def _as_datetime_index(nav_series: pd.Series) -> pd.Series:
    """将字符串日期索引转为 DatetimeIndex。"""
    if isinstance(nav_series.index, pd.DatetimeIndex):
        return nav_series
    return nav_series.copy()


def _as_datetime_index_vals(nav_series: pd.Series) -> pd.Series:
    """强制将索引转为 DatetimeIndex（供 resample 使用）。"""
    s = nav_series.copy()
