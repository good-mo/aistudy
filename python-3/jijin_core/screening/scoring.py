"""
综合评分模型
基于基金经理方法论的综合评分（满分 100），包含风格加成与周期动态阈值。
"""
...
def _calc_style_bonus(preferred_style: str, fund_scale: str) -> float:
    """根据偏好风格与基金规模计算风格加成。"""
    bonus = 0.0
    if preferred_style and "大盘" in preferred_style and fund_scale and "大" in fund_scale:
        bonus += 2.0
    if preferred_style and "小盘" in preferred_style and fund_scale and "小" in fund_scale:
        bonus += 2.0
    return bonus
...
def score_fund(
    ...
    *,
    ...
    cycle_thresholds: dict | None = None,
